...
from event_pack.event_simulator import *
import numpy as np
import airsim
import time
import cv2
...
class AirSimEventGen:
    def __init__(self, W, H, save=True, debug=False):
...
def publisher():
    global event
    event = Event()
    # defining the publisher by topic, message type
    pub = rospy.Publisher('talker', Event, queue_size=10)
    pub1 = rospy.Publisher('test', String, queue_size=10)

    # initialzing the ros node - "name it anything"
    rospy.init_node('publish_node', anonymous=True) 
    rate = rospy.Rate(10) # frequency in hertz of the publishing
